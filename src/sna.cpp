...
SNA::~SNA() { destroy_twojmax_arrays(); }
    ...
    parallel_for(
        policy, KOKKOS_CLASS_LAMBDA(const member_type team_member) {
            ...
            if (iter < nTotal) {
                ...
                sfac *= wj(natom, nbor);
#pragma omp atomic
                ulisttot(natom, 0).re += sfac * ulist(natom, nbor, 0).re;
#pragma omp atomic
                ulisttot(natom, 0).im += sfac * ulist(natom, nbor, 0).im;
                //                Kokkos::atomic_add(&(ulisttot(natom, 0).re),
                //                                   sfac * ulist(natom, nbor,
                //                                   0).re);
                ...
                for (int j = 1; j <= twojmax; j++) {
                    ...
                    for (int m_iter = 0; m_iter < m_max; ++m_iter) {
#pragma omp atomic
                        ulisttot(natom, jju).re +=
                            sfac * ulist(natom, nbor, jju).re;
#pragma omp atomic
                        ulisttot(natom, jju).im +=
                            sfac * ulist(natom, nbor, jju).im;
                        //                        Kokkos::atomic_add(&(ulisttot(natom,
                        //                        jju).re),
                        //                                           sfac *
                        //                                           ulist(natom,
                        //                                           nbor,
                    ...
                    for (int m_iter = 0; m_iter < m_max; ++m_iter) {
#pragma omp atomic
                        ulisttot(natom, jjup).re += sfac * ulist_parity(jju) *
                                                    ulist(natom, nbor, jju).re;
#pragma omp atomic
                        ulisttot(natom, jjup).im += sfac * ulist_parity(jju) *
                                                    -ulist(natom, nbor, jju).im;
                        //                        Kokkos::atomic_add(&(ulisttot(natom,
                        //                        jjup).re),
                        //                                           sfac *
        ...
        num_atoms * idxz_max, KOKKOS_CLASS_LAMBDA(const int iter) {
            ...
            for (int ib = 0; ib < nb; ib++) {
                ...
                for (int ia = 0; ia < na; ia++) {
                    suma1_r +=
                        cgblock[icga] * (ulisttot(natom, jju1 + ma1).re *
                    ...
                    for (int ma = 0; ma < j; ma++) {
                        ...
                        for (int k = 0; k < 3; k++) {
                            dulist(natom, nbor, jjdu, k).re +=
                                rootpq *
                                (da_r[k] * ulist(natom, nbor, jjup).re +
                            ...
                            dulist(natom, nbor, jjdu, k).im +=
                                rootpq *
                                (da_r[k] * ulist(natom, nbor, jjup).im -
                        ...
                        for (int k = 0; k < 3; k++) {
                            dulist(natom, nbor, jjdu + 1, k).re =
                                -rootpq *
                                (db_r[k] * ulist(natom, nbor, jjup).re +
                            ...
                            dulist(natom, nbor, jjdu + 1, k).im =
                                -rootpq *
                                (db_r[k] * ulist(natom, nbor, jjup).im -
...
void SNA::compute_duidrj() { compute_duarray(); }
...
------------------------------------------------------------------------- */
KOKKOS_INLINE_FUNCTION
void SNA::add_uarraytot(int natom, int nbor, double r, double wj, double rcut) {
...
------------------------------------------------------------------------- */
void SNA::compute_uarray(int natom, int nbor, double x, double y, double z,
                         double z0, double r) {
...
KOKKOS_INLINE_FUNCTION
double SNA::compute_sfac(double r, double rcut) const {
...
/* ---------------------------------------------------------------------- */
KOKKOS_INLINE_FUNCTION
double SNA::compute_dsfac(double r, double rcut) const {
